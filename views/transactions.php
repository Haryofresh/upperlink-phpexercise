            ...
            <tbody>
                ...
                <?php
                    for($i = 1; $i<$dataLength; $i++) {
                        ...
                        for ($m = 0; $m<$itemLength; $m++) {
                            if ($m === 0) {                 // check if 1st column and change date format if true
                                $arrDate = $data[$i][$m];
                                $arrDate = explode('/', $arrDate); // Split string
                                $setDate =  date("M d, Y", mktime(0,0,0,$arrDate[0],$arrDate[1],$arrDate[2]));
                                $singleRow .= "<td>". $setDate . "</td>";
                            }
                            elseif ($m !== $lastIndex) {        // check if item is not the last column
                                $singleRow .= "<td>". $data[$i][$m]."</td>";
                            } else {
                                if (str_starts_with($data[$i][$m], '-')) {      // To check if amount is an expense and color red
                                    $singleRow .= "<td class='expense'>". $data[$i][$m]."</td>";
                                } else {                                        // To check amount is an income and add color green
                                    $singleRow .= "<td class='income'>". $data[$i][$m]."</td>";
                                }
                            }
                        }
